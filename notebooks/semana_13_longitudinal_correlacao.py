...
def _(mo):
    mo.md(r"""
    # Semana 13 — Correlação longitudinal

    **Objetivo da semana:** Ver se crianças ativas permanecem mais ativas em idades posteriores.

    **Por que isso importa?** A fase 4 introduz o raciocínio longitudinal. O estudante compara medidas repetidas e discute estabilidade comportamental ao longo do tempo.
...
def _(df, mo, plt, sns, variavel_x):
    ...
    ax.set_title("Matriz temporal de " + variavel_x.value)
...
def _(df, mo, plt, sns, variavel_x):
    ...
    if coluna_grupo in df.columns:
        ...
        ax.set_title("Trajetórias individuais para " + variavel_x.value)
        componentes.extend([
