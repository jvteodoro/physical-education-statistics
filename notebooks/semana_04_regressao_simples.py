...
def _(mo):
    mo.md(r"""
    # Semana 4 — Regressão simples

    **Objetivo da semana:** Modelar atividade física a partir da competência motora usando regressão linear.

    **Por que isso importa?** Depois de observar associação, o estudante aprende a ajustar uma reta e interpretar intercepto, inclinação e R².
...
def _(df, mo, plt, sns, variavel_x):
    ...
    ax.set_title("Linha de regressão de " + variavel_x.value)
...
def _(df, mo, plt, sns, variavel_x):
    ...
    if coluna_grupo in df.columns:
        ...
        ax.set_title("Cenários previstos para " + variavel_x.value)
        componentes.extend([
