...
def _(mo):
    mo.md(r"""
    # Semana 14 — Trajetórias e tracking

    **Objetivo da semana:** Visualizar perfis individuais e médias por participação esportiva.

    **Por que isso importa?** Este notebook enfatiza visualização de mudança. Em vez de olhar apenas correlações, o estudante observa trajetórias, manutenção e queda da atividade física.
...
def _(df, mo, plt, sns, variavel_x):
    ...
    ax.set_title("Linhas no tempo de " + variavel_x.value)
...
def _(df, mo, plt, sns, variavel_x):
    ...
    if coluna_grupo in df.columns:
        ...
        ax.set_title("Médias por grupo para " + variavel_x.value)
        componentes.extend([
