...
def _():
    ...
    df = load_dataset("motor_learning")
    variable_options = [
            "tentativa_1",
            "tentativa_5",
            "retencao",
...
def _(mo):
    mo.md(r"""
    # Semana 16 — Síntese e projeto final

    **Objetivo da semana:** Conectar evidências do curso em uma análise de aprendizagem motora e planejar o projeto final.

    **Por que isso importa?** A última semana fecha o ciclo com uma base experimental simples. O estudante revisa comparação de grupos, efeito de intervenção e planejamento de um mini-projeto replicável.
...
def _(mo, variable_options):
    variavel_x = mo.ui.dropdown(options=variable_options, value="tentativa_1", label="Variável principal")
    variavel_y = mo.ui.dropdown(options=variable_options, value="tentativa_5", label="Variável secundária")
...
def _(df, mo, plt, sns, variavel_x):
    ...
    ax.set_title("Evolução por grupo de " + variavel_x.value)
...
@app.cell
def _(mo):
    filtro_grupo = mo.ui.dropdown(options=["Todos", "grupo"] if "grupo" in ["sexo", "participa_esporte", "grupo", "idade"] else ["Todos"], value="Todos", label="Agrupar análise?")
    return filtro_grupo
...
def _(df, mo, plt, sns, variavel_x):
    ...
    if coluna_grupo in df.columns:
        ...
        ax.set_title("Roteiro de projeto para " + variavel_x.value)
        componentes.extend([
