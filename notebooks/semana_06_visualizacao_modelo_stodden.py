...
def _():
    ...
    df = load_dataset("motor_competence")
    variable_options = [
            "competencia_motora",
            "aptidao_fisica",
            "atividade_fisica",
...
def _(mo):
    mo.md(r"""
    # Semana 6 — Modelo de Stodden em dados

    **Objetivo da semana:** Explorar o encadeamento entre competência motora, aptidão física e atividade física.

    **Por que isso importa?** O notebook usa gráficos e resumos por quartis para aproximar o raciocínio teórico do modelo de Stodden. O objetivo é enxergar caminhos plausíveis entre variáveis.
...
def _(mo, variable_options):
    variavel_x = mo.ui.dropdown(options=variable_options, value="competencia_motora", label="Variável principal")
    variavel_y = mo.ui.dropdown(options=variable_options, value="aptidao_fisica", label="Variável secundária")
...
def _(df, mo, plt, sns, variavel_x):
    ...
    ax.set_title("Mapa de relações de " + variavel_x.value)
...
def _(df, mo, plt, sns, variavel_x):
    ...
    if coluna_grupo in df.columns:
        ...
        ax.set_title("Grupos por quartis para " + variavel_x.value)
        componentes.extend([
