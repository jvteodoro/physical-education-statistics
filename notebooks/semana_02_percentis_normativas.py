...
def _(mo):
    ...
    **Objetivo da semana:** Construir curvas de referência por idade e percentis para interpretar desempenho motor.

    **Por que isso importa?** Aqui o estudante aprende a transformar dados brutos em referências normativas. A proposta é comparar crianças de idades diferentes sem perder o contexto do desenvolvimento.
...
def _(mo, variable_options):
    variavel_x = mo.ui.dropdown(options=variable_options, value="escore_motor_total", label="Variável principal")
    variavel_y = mo.ui.dropdown(options=variable_options, value="correr", label="Variável secundária")
...
def _(df, mo, plt, sns, variavel_x):
    ...
    ax.set_title("Curvas percentílicas de " + variavel_x.value)
...
@app.cell
def _(mo):
    filtro_grupo = mo.ui.dropdown(options=["Todos", "idade"] if "idade" in ["sexo", "participa_esporte", "grupo", "idade"] else ["Todos"], value="Todos", label="Agrupar análise?")
    return filtro_grupo
...
def _(df, mo, plt, sns, variavel_x):
    ...
    if coluna_grupo in df.columns:
        ...
        ax.set_title("Tabela de referência para " + variavel_x.value)
        componentes.extend([
