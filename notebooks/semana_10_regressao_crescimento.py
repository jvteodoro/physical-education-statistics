...
def _(mo):
    mo.md(r"""
    # Semana 10 — Regressão em crescimento

    **Objetivo da semana:** Modelar desempenho físico com idade, maturação e altura.

    **Por que isso importa?** Este notebook operacionaliza a pergunta central da fase 3: idade cronológica explica tudo? A comparação entre preditores ajuda a discutir desenvolvimento e maturação.
...
def _(df, mo, plt, sns, variavel_x):
    ...
    ax.set_title("Coeficientes ajustados de " + variavel_x.value)
...
def _(df, mo, plt, sns, variavel_x):
    ...
    if coluna_grupo in df.columns:
        ...
        ax.set_title("Predição por idade para " + variavel_x.value)
        componentes.extend([
