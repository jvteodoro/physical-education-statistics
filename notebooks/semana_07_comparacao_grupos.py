...
def _(mo):
    mo.md(r"""
    # Semana 7 — Comparação entre grupos

    **Objetivo da semana:** Comparar atividade física entre grupos de alta e baixa competência motora.

    **Por que isso importa?** Nesta semana o estudante pratica recodificação, criação de grupos e comparação de distribuições, preparando o terreno para análises aplicadas em artigos.
...
def _(df, mo, plt, sns, variavel_x):
    ...
    ax.set_title("Boxplot de " + variavel_x.value)
...
def _(df, mo, plt, sns, variavel_x):
    ...
    if coluna_grupo in df.columns:
        ...
        ax.set_title("Resumo por grupo para " + variavel_x.value)
        componentes.extend([
