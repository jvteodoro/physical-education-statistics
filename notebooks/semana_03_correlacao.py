...
def _(mo):
    ...
    **Objetivo da semana:** Investigar se maior competência motora está associada a mais atividade física.

    **Por que isso importa?** Esta semana introduz associação linear. O foco didático é diferenciar correlação de causalidade e aprender a ler a força e o sinal de r de Pearson.
...
def _(mo, variable_options):
    ...
    variavel_y = mo.ui.dropdown(options=variable_options, value="atividade_fisica", label="Variável secundária")
...
def _(df, mo, plt, sns, variavel_x):
    ...
    ax.set_title("Dispersão com tendência de " + variavel_x.value)
...
def _(df, mo, plt, sns, variavel_x):
    ...
    if coluna_grupo in df.columns:
        ...
        ax.set_title("Matriz de correlação para " + variavel_x.value)
        componentes.extend([
