...
def _(mo):
    mo.md(r"""
    # Semana 8 — Predição e interpretação

    **Objetivo da semana:** Criar regras simples para identificar participantes com menor atividade física.

    **Por que isso importa?** A ideia aqui é traduzir estatística em tomada de decisão. O notebook mostra como limiares e percentis podem apoiar triagens iniciais sem prometer diagnóstico.
...
def _(mo, variable_options):
    variavel_x = mo.ui.dropdown(options=variable_options, value="atividade_fisica", label="Variável principal")
    variavel_y = mo.ui.dropdown(options=variable_options, value="competencia_motora", label="Variável secundária")
...
def _(df, mo, plt, sns, variavel_x):
    ...
    ax.set_title("Faixas de risco de " + variavel_x.value)
...
def _(df, mo, plt, sns, variavel_x):
    ...
    if coluna_grupo in df.columns:
        ...
        ax.set_title("Exploração de limiares para " + variavel_x.value)
        componentes.extend([
