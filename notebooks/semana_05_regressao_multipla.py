...
def _(mo):
    mo.md(r"""
    # Semana 5 — Regressão múltipla

    **Objetivo da semana:** Ver se competência motora continua importante após controlar idade, sexo e IMC.

    **Por que isso importa?** A fase 2 começa com modelos mais realistas. O estudante testa como variáveis de confusão alteram a interpretação do efeito principal.
...
def _(df, mo, plt, sns, variavel_x):
    ...
    ax.set_title("Efeito ajustado de " + variavel_x.value)
...
def _(df, mo, plt, sns, variavel_x):
    ...
    if coluna_grupo in df.columns:
        ...
        ax.set_title("Comparação de modelos para " + variavel_x.value)
        componentes.extend([
