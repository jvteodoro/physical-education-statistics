...
def _():
    ...
    df = load_dataset("longitudinal_activity")
    variable_options = [
            "atividade_8_anos",
            "atividade_10_anos",
            "atividade_12_anos",
...
def _(mo):
    mo.md(r"""
    # Semana 15 — Introdução a modelos mistos

    **Objetivo da semana:** Usar uma versão simplificada de dados longos para estimar tendência média no tempo.

    **Por que isso importa?** Mesmo para iniciantes, é útil conhecer a lógica de modelos mistos. O notebook explica a ideia sem exigir matemática pesada, priorizando interpretação.
...
def _(mo, variable_options):
    variavel_x = mo.ui.dropdown(options=variable_options, value="atividade_8_anos", label="Variável principal")
    variavel_y = mo.ui.dropdown(options=variable_options, value="atividade_10_anos", label="Variável secundária")
...
def _(df, mo, plt, sns, variavel_x):
    ...
    ax.set_title("Modelo com tempo de " + variavel_x.value)
...
@app.cell
def _(mo):
    filtro_grupo = mo.ui.dropdown(options=["Todos", "participa_esporte"] if "participa_esporte" in ["sexo", "participa_esporte", "grupo", "idade"] else ["Todos"], value="Todos", label="Agrupar análise?")
    return filtro_grupo


@app.cell
def _(df, mo, plt, sns, variavel_x):
    coluna_grupo = "participa_esporte"
    ...
    if coluna_grupo in df.columns:
        ...
        ax.set_title("Formato wide vs. long para " + variavel_x.value)
        componentes.extend([
