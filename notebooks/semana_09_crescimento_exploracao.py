...
def _(mo):
    mo.md(r"""
    # Semana 9 — Crescimento e maturação

    **Objetivo da semana:** Descrever idade, altura, massa e desempenho físico ao longo da adolescência.

    **Por que isso importa?** Começa a fase de crescimento com foco em descrição e visualização. O estudante aprende a relacionar maturação biológica com diferenças entre adolescentes.
...
def _(mo, variable_options):
    variavel_x = mo.ui.dropdown(options=variable_options, value="idade", label="Variável principal")
    variavel_y = mo.ui.dropdown(options=variable_options, value="altura_cm", label="Variável secundária")
...
def _(df, mo, plt, sns, variavel_x):
    ...
    ax.set_title("Trajetórias por idade de " + variavel_x.value)
...
def _(df, mo, plt, sns, variavel_x):
    ...
    if coluna_grupo in df.columns:
        ...
        ax.set_title("Resumo por sexo para " + variavel_x.value)
        componentes.extend([
