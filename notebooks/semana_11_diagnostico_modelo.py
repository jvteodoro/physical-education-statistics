...
def _(mo):
    mo.md(r"""
    # Semana 11 — Diagnóstico de modelos

    **Objetivo da semana:** Inspecionar resíduos, valores previstos e possíveis violações de pressupostos.

    **Por que isso importa?** Além de ajustar um modelo, é preciso verificar se ele faz sentido. O estudante aprende a desconfiar do resultado antes de reportá-lo.
...
def _(df, mo, plt, sns, variavel_x):
    ...
    ax.set_title("Resíduos de " + variavel_x.value)
...
def _(df, mo, plt, sns, variavel_x):
    ...
    if coluna_grupo in df.columns:
        ...
        ax.set_title("Normalidade e homocedasticidade para " + variavel_x.value)
        componentes.extend([
